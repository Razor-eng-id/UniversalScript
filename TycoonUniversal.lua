...
local function findTycoonFolder()
	local workspace = game:GetService("Workspace")
	local player = LocalPlayer

	-- Cari folder tycoon milik player berdasarkan nama
	for _, obj in pairs(workspace:GetDescendants()) do
		if obj:IsA("Model") or obj:IsA("Folder") then
			local name = obj.Name:lower()
			if name:find(player.Name:lower()) or name:find("tycoon") then
				return obj
			end
		end
	end

	-- Fallback: cari semua tycoon
	for _, obj in pairs(workspace:GetChildren()) do
		if obj:IsA("Model") or obj:IsA("Folder") then
			if obj.Name:lower():find("tycoon") then
				return obj
			end
		end
	end
...
local function getCollectPads()
	local pads = {}
	local root = findTycoonFolder()

	for _, obj in pairs(root:GetDescendants()) do
		if obj:IsA("BasePart") or obj:IsA("MeshPart") or obj:IsA("UnionOperation") then
			local name = obj.Name:lower()
			if name:find("collect") or name:find("drop") or name:find("cash") or name:find("money") or name:find("income") or name:find("collect") then
...
local function getUpgradePads()
	...
	for _, obj in pairs(root:GetDescendants()) do
		if obj:IsA("BasePart") or obj:IsA("MeshPart") or obj:IsA("UnionOperation") then
			local name = obj.Name:lower()
			if name:find("buy") or name:find("upgrade") or name:find("purchase") or name:find("unlock") or name:find("button") then
				table.insert(pads, obj)
			end
		end
	end
